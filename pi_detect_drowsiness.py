...
# ==================  Azure & Firebase Setup ===================
# Azure Api Setup
os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = 'admin.json'
KEY = 'a6a8584b92574245aef6f0b872712723'  
CF.Key.set(KEY)
...
bucket = client.get_bucket('makentu-c3a48.appspot.com')

# Adrian alarm setting
...
def get_faceId(image_path):
	result = CF.face.detect(image_path)
	if len(result) == 0:
	    return None
	return result[0]['faceId']

owner_image_path = 'owner.jpg'
owner_faceId = get_faceId(owner_image_path)
...
# frame count to send to Microsoft Azure Cloud platform and do face detection
frame_cycle = 30
frame_count = frame_cycle-1

# ===================
# define RPi.GPIO input and output
# ===================
...
while True:
	...
	# channels)
	frame = vs.read()
	if frame_count % frame_cycle == frame_cycle-1:
	    frame_count = 0
	    cv2.imwrite("face.jpg", frame)
	    query_faceId = get_faceId('face.jpg')
	    if query_faceId is not None:
	        result = CF.face.verify(owner_faceId, query_faceId)
	        if not result['isIdentical']:  # Unidentified Person!!!!!!
	            print("======   THIEF!!!!!   ======")
	            blob = bucket.blob('images/cover.jpg')
	            blob.upload_from_filename(filename='face.jpg')
